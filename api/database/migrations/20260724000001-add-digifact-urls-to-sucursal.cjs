...
module.exports = {
  ...
  async down(queryInterface) {
    const columns = await queryInterface.describeTable('sucursal');
    if (columns.auth_url_digifact) await queryInterface.removeColumn('sucursal', 'auth_url_digifact');
    if (columns.cert_url_digifact) await queryInterface.removeColumn('sucursal', 'cert_url_digifact');
    if (columns.cancel_url_digifact) await queryInterface.removeColumn('sucursal', 'cancel_url_digifact');
  }
};
